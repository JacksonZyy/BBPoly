...
specLB = None
specUB = None
assert dataset == 'cifar10', "The residual networks are for Cifar10!!!"
...
for i, test in enumerate(tests):
    ...
    eran_result = eran.analyze_box(specLB, specUB, init_domain('deeppoly'), config.timeout_lp, config.timeout_milp, config.use_default_heuristic, False, True, True, 3, False, 0, True, False)
    # Using input summary method to check the classification network of original image
    ...
    if(dominant_class == actual_label):
        ...
        with open(lb_fullpath, 'a+', newline='') as write_obj:
            ...
            csv_writer.writerow([net_file, str(dataset), "img "+str(i)+" with label "+str(actual_label), "eps="+str(epsilon), "inpsum", str(end - start)+" secs", str(dominant_class)])

        start = time.time()
        eran_result = eran.analyze_box(specLB, specUB, init_domain('deeppoly'), config.timeout_lp, config.timeout_milp, config.use_default_heuristic, False, True, True, 3, True, 4, False, False)
        end = time.time()
        dominant_class = eran_result[0]
        if(dominant_class == actual_label):
        ...
        with open(lb_fullpath, 'a+', newline='') as write_obj:
            csv_writer = csv.writer(write_obj)
            csv_writer.writerow([net_file, str(dataset), "img "+str(i)+" with label "+str(actual_label), "eps="+str(epsilon), "blksum_4bound", str(end - start)+" secs", str(dominant_class)])
                
print(candi_count, blksum_count, inpsum_count)
with open(lb_fullpath, 'a+', newline='') as write_obj:
    csv_writer = csv.writer(write_obj)
    csv_writer.writerow([candi_count, blksum_count, inpsum_count])